...
import { useActivities, useLanguages, ActivityType, calcStreak, minutesOnDate, todayKey } from "@/lib/store";
      ...
      {isEmpty ? (
        ...
        <>
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            ...
            <Stat icon={<ListChecks className="h-4 w-4" />} label={t("overall.totalSessions")} value={String(sessions)} />
            <Stat icon={<LangIcon className="h-4 w-4" />} label={t("overall.activeLangs")} value={String(activeLangs)} />
            <Stat icon={<Flame className="h-4 w-4 text-accent" />} label={t("overall.bestStreak")} value={`${bestStreak} ${t("common.days")}`} />
          </motion.div>
